	...
	unsigned char  hour, unsigned char  minute)
{
    ...
    if ( ss == 1 )
    {
        ...
        alarm.al[idx].hour = hour; 
        alarm.al[idx].minute = minute;
...
int SetAlarm( const unsigned char *buf )
{
    ...
    for ( int i = 0; i < 5; i++ )
    {
        ...
        if ( __SetAlarm( i, ss, hour, min ) != 0x0 )
        {
            return -1;
        }
    }
...
{
    unsigned long alarm_time_to_min_today[5];
    unsigned long alarm_time_to_min_tomorrow[5];
    ...
    if ( (alarm.alarm_s & (AEN(0)|(AEN(1)|AEN(2)|AEN(3)|AEN(4)|AEN(5)))) != 0 )
    {
