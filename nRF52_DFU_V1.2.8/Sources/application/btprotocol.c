...
int bt_protocol_rx(const unsigned char *buf, int length )
{
    ...
    case 0x28:
            ...
            }else if ( buf[2] == 0x3 )    // 紫外线报警设置
            {    
                if ( buf[3] == 0 ) //设置
                {
                    uit_warnning_enable = buf[4] & 0x1;
                    uit_warnning_val = buf[5]; // 强度
                    uit_warnning_interval = buf[6];// 报警间隔
                }else{
                    // 取消本次报警
                }
            }else if ( buf[2] == 0x80 )   // 打开或关闭数据上报功能.
            {
                ...
                if ( uit_notify_enable )
                {
                    osal_start_timerEx ( taskUItravioletTaskId, TASK_UIT_REPORT_UIT_EVT, 500 );
                }
            }else if ( buf[2] == 0x81) {
