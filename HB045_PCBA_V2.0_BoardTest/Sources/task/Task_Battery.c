...
short battery_level_arr[63];

#include "LookTable.h"

static float battery_get_level(unsigned short sample_adc)
{
    short max_value = 0x030A;   /* 4.17 V */
    short min_value = 0x023C;   /* 3.00 V */
	...
	power.voltage = 3.0 + (4.17-3.0) * p / 100.0;
	float battery_level = getBattery(power.voltage);	
	#endif
	
	unsigned char level;
	if ( battery_level > 4 ) {
		level = (battery_level - 4) * 100.0f/(100-4);
	}else{
		level = 0;
	}	
...
unsigned long taskBattery( unsigned long task_id, unsigned long events )
{
	...
	{	
		...
		osal_start_timerEx( task_id, TASK_BATTERY_CHARGE_EVT, 500);  
#if (OTA_MODE_ENABLE==0)		
		osal_start_timerEx( task_id, TASK_BATTERY_COMFIRM_EVT, 2600);
#endif
		return ( events ^ TASK_BATTERY_INIT_EVT ); 
	}
	
#if (OTA_MODE_ENABLE==0)	
	if ( events & TASK_BATTERY_COMFIRM_EVT )
	{
		if ( current_adc < 360 || current_adc > 900 )
		{
			power.ok = 0;
		}
		
		return ( events ^ TASK_BATTERY_COMFIRM_EVT );
	}
#endif
