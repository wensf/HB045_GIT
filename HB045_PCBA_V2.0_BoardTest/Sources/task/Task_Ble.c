...
#define APP_ADV_INTERVAL                MSEC_TO_UNITS(500, UNIT_0_625_MS)           /**< The advertising interval (in units of 0.625 ms. This value corresponds to 25 ms). */
...
static void advertising_init(void)
{
	...
	m_addl_adv_manuf_data[0] = 0x00;
	m_addl_adv_manuf_data[1] = 0x00;
...
unsigned long taskBle( unsigned long task_id, unsigned long events )
{
	...
	if ( events & TASK_BLE_DISCONNECTED_EVT )
	{
        ...
        power.debug = 0;
#if 1		
		Log_Printf( LOG_FD_BLE, "disconnect" );
#endif
		osal_set_event ( taskUploadTaskId, TASK_UPLOAD_RESET_QUEUE_EVT);
	...
	if ( events & TASK_BLE_MODIFY_DEVICE_NAME_EVT )
	{
		ble_gap_conn_sec_mode_t sec_mode;
		...
		return ( events ^ TASK_BLE_MODIFY_DEVICE_NAME_EVT );
	}
