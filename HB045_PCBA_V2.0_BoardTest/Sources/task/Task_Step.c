...
unsigned long taskStep( unsigned long task_id, unsigned long events )
{
	...
	{ 
		...
		osal_start_timerEx( task_id, TASK_STEP_BMA_RE_INIT_EVT, 4000 ); 
    ...
    ***************************************************************************/
	...
	if ( events & TASK_STEP_UPLOAD_PEDOMETER_EVT )
	{
        ...
        {       
            ...
            if ( (accelDevice.allCounter - accelDevice.oCounter) > 14)
            {
                stepStatusMachine = 1;              
	...
	if ( events & TASK_STEP_BMA_INIT_EVT )
	{
		...
		osal_start_timerEx( task_id, TASK_STEP_BMA_RE_INIT_EVT, 2500 );
	...
	if ( events & STEP_TASK_INT_COMFIRM_EVT )
	{
		if ( (taskStepInterrutCount > 2) && (taskStepInterrutCount < 6) )
		{
            ...
            osal_start_timerEx ( task_id, TASK_STEP_BMA_RE_INIT_EVT,100 );
		}
