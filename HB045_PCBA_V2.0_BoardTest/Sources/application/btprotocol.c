...
int bt_protocol_rx(const unsigned char *buf, int length )
{
    ...
    case 0xC: /* Request sleep data */
            ...
            }else if ( buf[2] == 0x03 ) // generate sleep test data
			{
	...
	case 0x0F: /* Firmware maintenance */
			...
			if ( buf[2] == 0x7 )
			{
				...
				if ( (buf[3] == 0) || (buf[3] > DEVICE_NAME_LENGTH) )
				{
					txbuf[0] |= 0x1<<7;
				}else{
					// 复制用户设置的名称到配置变量中
					// 置位 TASK_BLE_MODIFY_DEVICE_NAME_EVT 执行修改动作
					memset(config.device_name, 0, sizeof(config.device_name));
					strncpy(config.device_name, (char*)buf+4, buf[3]);
					osal_set_event ( taskBleTaskId, TASK_BLE_MODIFY_DEVICE_NAME_EVT );
...
int handle_notification( const unsigned char *buf, unsigned char *txbuf)
{
	...
	if ( buf[2] == 0xF0 )
	{
		...
		txbuf[0] = 0x08;
		txbuf[1] = ble.ancs_notify_enable;
		txbuf[2] = 0;
